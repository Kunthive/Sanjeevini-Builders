  ...
  return (
    <>
      <button
        ...
        className="fixed top-6 left-6 z-50 p-3 bg-primary text-primary-foreground rounded-lg hover:shadow-lg transition-all duration-300 hover:scale-110 md:hidden"
      >
        {isOpen ? <Icons.X size={24} /> : <Icons.Menu size={24} />}
      </button>

      <Link href="/" className="fixed top-6 left-6 z-40 flex items-center gap-2 group hidden md:flex">
      ...
      {isOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute top-20 left-6 right-6 bg-white rounded-lg shadow-2xl p-6 space-y-4 animate-fade-in-up">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
